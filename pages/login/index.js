...
Page({
  ...
  binQuery() {
    ...
    app.api.post("250:8019/api/user/account/binQuery", data).then((res) => {
      if (res.respCode == "00000000") {
        if (res.bindNum == 0 || res.bindNum == undefined) {
          //未绑卡||未注册
          this.toRegister();
        } else {
          this.toSsmm();
  ...
  getPhoneInfo() {
    wx.getSystemInfo({
      success: (res) => {
        wx.setStorageSync("phoneInfo", res);
      },
    });
  },
  // 登录、注册
  goLoginRegister() {
    ...
    if (
      this.data.isRegister != -1 &&
      this.data.mobilePhone.length == 11 &&
      ...
      app.api
        .post("pweb/perLoginticWx.do", data, header)
        .then((res) => {
          if (res.respCode == "00000000") {
  ...
  getOpenId() {
    ...
    if (info == "" || info == undefined) {
      wx.login({
        success: (res) => {
          app.api
            .post("pweb/perWxGetOpenId.do", { code: res.code })
            .then((res) => {
