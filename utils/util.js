...
const getDay = day => {
    ...
    tMonth = (doHandleMonth(tMonth + 1) < 10 ? '0' + doHandleMonth(tMonth + 1) : doHandleMonth(tMonth + 1));
    tDate = (doHandleMonth(tDate) < 10 ? '0' + doHandleMonth(tDate) : doHandleMonth(tDate));
...
const w = () => {
    ...
    if (day - 7 <= 0) { //如果在当月7日之前
        ...
        if (month - 1 <= 0) { //如果在当年的1月份
            ...
            dateObj.start = year + '-' + jia((month - 1)) + '-' + jia((startMonthDay - (7 - day)));
        }
    } else {
    ...
    if (month - 1 <= 0) { //如果是1月，年数往前推一年<br>　　　　
        ...
        if (startMonthDay < day) { //1个月前所在月的总天数小于现在的天日期
            if (day < endMonthDay) { //当前天日期小于当前月总天数
                dateObj.start = year + '-' + jia((month - 1)) + '-' + jia((startMonthDay - (endMonthDay - day)));
            } else {
...
const tm = () => {
    ...
    if (month - 3 <= 0) { //如果是1、2、3月，年数往前推一年
        ...
        if (start3MonthDay < day) { //3个月前所在月的总天数小于现在的天日期
            dateObj.start = (year - 1) + '-' + jia((12 - (3 - month))) + '-' + jia(start3MonthDay);
        } else {
        ...
        if (start3MonthDay < day) { //3个月前所在月的总天数小于现在的天日期
            if (day < endMonthDay) { //当前天日期小于当前月总天数,2月份比较特殊的月份
                dateObj.start = year + '-' + jia((month - 3)) + '-' + jia((start3MonthDay - (endMonthDay - day)));
            } else {
...
const hiddenBankCard = (bankNumber) => {
    ...
    let s3 = bankNumber.substring(l1 - 10).replace(/[0-9]/ig, "*")
